...
const NGOLogin = () => {
  ...
  const login = useNgoStore((s) => s.login);
  ...
  useEffect(() => {
    ...
    document.title = 'NGO Login | ZeroWasteBite';
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const ngo = await login(email, password);
      if (!ngo) throw new Error('Login failed');
      ...
      >
        <div className="p-8">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-primary-content">NGO Sign in</h1>
            <p className="text-sm text-gray-600 mt-1">Access your NGO portal</p>
          </div>
          ...
          <p className="mt-6 text-center text-sm text-gray-600">
            New NGO?{' '}
            <Link to="/ngo/register" className="text-secondary hover:text-secondary-dark font-semibold">Create an account</Link>
          </p>
        </div>
      </motion.div>
    </div>
