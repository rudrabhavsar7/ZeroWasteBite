...
import { IconUser, IconMail, IconLock, IconHome, IconArrowRight, IconChecks } from '@tabler/icons-react';
import useNgoStore from '../../app/ngoStore';
import toast from 'react-hot-toast';

const NGORegister = () => {
  const navigate = useNavigate();
  const loading = useNgoStore((s) => s.loading);
  const setLoading = useNgoStore((s) => s.setLoading);
  const register = useNgoStore((s) => s.register);
  ...
  useEffect(() => {
    ...
    document.title = 'NGO Register | ZeroWasteBite';
  ...
  const validate = () => {
    if (!name || !email || !phone || !password || !confirm || !organizationName || !street || !city || !state || !zip) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      await register({ name, email, phone, password, coordinates, organizationName, street, city, state, zip });
      ...
      >
        <div className="p-8">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-primary-content">Create your <span className="text-secondary font-semibold">NGO</span> account</h1>
            <p className="text-sm text-gray-600 mt-1">Register your organization to receive donations.</p>
          </div>

          <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="md:col-span-1">
              <label htmlFor="name" className="block text-sm font-medium text-primary-content mb-1">Contact person</label>
            ...
            <div className="md:col-span-1">
              <label htmlFor="organizationName" className="block text-sm font-medium text-primary-content mb-1">Organization name</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-primary-content/60"><IconHome size={20} /></span>
                <input id="organizationName" type="text" value={organizationName} onChange={(e) => setOrganizationName(e.target.value)} className="w-full rounded-lg border border-primary-content/20 focus:border-secondary focus:ring-2 focus:ring-secondary/40 outline-none px-10 py-3 text-primary-content placeholder:text-gray-400" placeholder="Your NGO" />
              </div>
            </div>

            <div className="md:col-span-1">
              <label htmlFor="street" className="block text-sm font-medium text-primary-content mb-1">Street</label>
              <input id="street" type="text" value={street} onChange={(e) => setStreet(e.target.value)} className="w-full rounded-lg border border-primary-content/20 focus:border-secondary focus:ring-2 focus:ring-secondary/40 outline-none px-4 py-3 text-primary-content" />
            </div>

            <div className="md:col-span-1">
              <label htmlFor="city" className="block text-sm font-medium text-primary-content mb-1">City</label>
              <input id="city" type="text" value={city} onChange={(e) => setCity(e.target.value)} className="w-full rounded-lg border border-primary-content/20 focus:border-secondary focus:ring-2 focus:ring-secondary/40 outline-none px-4 py-3 text-primary-content" />
            </div>

            <div className="md:col-span-1">
              <label htmlFor="state" className="block text-sm font-medium text-primary-content mb-1">State</label>
              <input id="state" type="text" value={state} onChange={(e) => setState(e.target.value)} className="w-full rounded-lg border border-primary-content/20 focus:border-secondary focus:ring-2 focus:ring-secondary/40 outline-none px-4 py-3 text-primary-content" />
            </div>

            <div className="md:col-span-1">
              <label htmlFor="zip" className="block text-sm font-medium text-primary-content mb-1">ZIP / Postal code</label>
              <input id="zip" type="number" value={zip} onChange={(e) => setZip(e.target.value)} className="w-full rounded-lg border border-primary-content/20 focus:border-secondary focus:ring-2 focus:ring-secondary/40 outline-none px-4 py-3 text-primary-content" />
            </div>
          ...
          <p className="mt-6 text-center text-sm text-gray-600">Already have an account? <Link to="/ngo/login" className="text-secondary hover:text-secondary-dark font-semibold">Sign in</Link></p>
        </div>
      </motion.div>
    </div>
  );
